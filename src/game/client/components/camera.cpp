...
CCamera::CCamera()
{
	...
	m_Positions[POS_SETTINGS_PWN] = vec2(1100.0f, 1000.0f);
...
{
    // zoom bind
    if(Input()->KeyIsPressed(KEY_LCTRL) && Input()->KeyPress(KEY_MOUSE_WHEEL_UP)) {
        m_Zoom *= ZoomStep;
    }
    else if(Input()->KeyIsPressed(KEY_LCTRL) && Input()->KeyPress(KEY_MOUSE_WHEEL_DOWN)) {
        m_Zoom *= 1 / ZoomStep;
    }
    else if(Input()->KeyIsPressed(KEY_LCTRL) && Input()->KeyPress(KEY_MOUSE_3)) {
        OnReset();
    }
