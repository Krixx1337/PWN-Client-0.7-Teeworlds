...
public:
	...
	bool CheckPoint(float x, float y, int Flag=TILE_SOLID) const { return IsTile(round_to_int(x), round_to_int(y), Flag); }
	bool CheckPoint(vec2 Pos, int Flag=TILE_SOLID) const { return CheckPoint(Pos.x, Pos.y, Flag); }
	...
	bool TestBox(vec2 Pos, vec2 Size, int Flag=TILE_SOLID) const;
